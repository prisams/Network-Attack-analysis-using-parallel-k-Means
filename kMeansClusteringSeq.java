...
/**
 * Class kMeansClusteringSeq is a sequential program to form  
 * clusters of different network attack types
 * <P>
 * Usage: java pj2 kMeansClusteringSeq <DataSet> <seed>
...
public class kMeansClusteringSeq extends Task {

	//array list to store all the data points
	static ArrayList<Features> dataPoints = new ArrayList<Features>();
	//Hashmap to store the final clusters
	static HashMap<Integer,ArrayList<Integer>> finalClusterMap;

	static double SSE=Double.MAX_VALUE;
	...
	public void main(String args[]) throws Exception{
		...
		try {
			...
			while((line=in.readLine())!=null){
				...
				dataPoints.add(features_norm);
			}

			//perform clustering for 100 iterations 
			for(int i=0; i <100 ;i++){
				cluster(4);
			}
			//to label the final cluster
			findAttackMappings(finalClusterMap);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void cluster(int k){
		...
		while(true){
			...
			for (int i=0;i<k;i++){
				Features new_centroid= getNewCentroid(pointClusterMap.get(i));
		...
		//distances of points in a cluster with their centroids
		for(int i=0;i<centroid_list.size();i++){
		...
		if(distance < SSE){
			SSE = distance;
			finalClusterMap = pointClusterMap;
		}
	}
	 ...
	 */	
	public static Features getNewCentroid(ArrayList<Integer> mapValue){
	...
	(HashMap<Integer,ArrayList<Integer>> finalClusters){
		...
		for (int i=0;i<finalClusters.size();i++){
			...
			//printing the occurence of each attack type in a cluster
	...
	private static void usage()
	{
		System.err.println ("Usage: java pj2 " +
				"kMeansClusteringSeq <DataSet> <seed>");
